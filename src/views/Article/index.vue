...
// 因为只有这一块才需要用这个样式来渲染，所以没有必要在全局main.js中引入
...
export default {
  ...
  async created () {
    ...
    // 在这块的时候loading关闭了 页面就显示正文 单必须写在nextTick
    // 因为数据更新 视图不能立即更新 写在 nextTick 回调中可以  因为nextTick回调就是在视图更新完成 才会触发
    this.$nextTick(() => {
      // 获取正文中的图片
      const arr = document.querySelectorAll('.article-content img')
      if (arr.length === 0) return // 防御代码 如果没有图片直接返回
      const arr1 = [] // 放图片src值
